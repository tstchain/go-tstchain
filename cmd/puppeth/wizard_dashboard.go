...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"tststats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "tststats":
				if infos, err := checkTststats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have tststats running, ask whtster to make the secret public or not
	if w.conf.tststats != "" {
		fmt.Println()
		fmt.Println("Include tststats secret on dashboard (y/n)? (default = yes)")
