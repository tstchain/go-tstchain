...
// PeerInfo represents a short summary of the Tstchain sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Tstchain protocol version negotiated
...
// Handshake executes the tst protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("tst/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Tstchain sub-protocol.
type peerSet struct {
