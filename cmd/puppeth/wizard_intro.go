...
// makeWizard creates and returns a new pupptst wizard.
...
// setting up a new or managing an existing Tstchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupptst, your Tstchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Tstchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and tststats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupptst uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Tstchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupptst", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somtsting I can do")
		}
	}
}
