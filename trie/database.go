...
type DatabaseReader interface {
	...
	// Has retrieves whtster a key is present in the database.
...
type Database struct {
	diskdb tstdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb tstdb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) commit(hash common.Hash, batch tstdb.Batch) error {
	// If the node does not exist, it's a previously committed node
