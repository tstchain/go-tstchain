...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Tstchain
...
type wallet struct {
	...
	deriveChain    tstereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whtster they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Tstchain account
			...
			// If the next account is empty, stop self-derivation, but add it nontsteless
...
// Contains implements accounts.Wallet, returning whtster a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain tstereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
