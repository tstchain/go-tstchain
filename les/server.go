...
func NewLesServer(tst *tst.Tstchain, config *tst.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(tst.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, tst.EventMux(), tst.Engine(), newPeerSet(), tst.BlockChain(), tst.TxPool(), tst.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(tst.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(tst.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(tst.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(tst.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(tst.ChainDb())
	return srv, nil
}
...
func newCostStats(db tstdb.Database) *requestCostStats {
