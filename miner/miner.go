...
// Package miner implements Tstchain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whtster we can start the mining operation
	shouldStart int32 // should start indicates whtster we should start after sync
}

func New(tst Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		tst:      tst,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, tst, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(tst.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setTsterbase(coinbase)
...
func (self *Miner) SetTsterbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setTsterbase(addr)
}
