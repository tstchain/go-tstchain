...
import (
	...
	"github.com/tstchain/go-tstchain/internal/jsre"
	"github.com/tstchain/go-tstchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Tstchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jtst", struct{}{})

	jtstObj, _ := c.jsre.Get("jtst")
	jtstObj.Object().Set("send", bridge.Send)
	jtstObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jtst);"); err != nil {
	...
	flatten := "var tst = web3.tst; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = tst.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jtst.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jtst.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jtst.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jtst.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jtst.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines tst.getBalance(tst.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gtst instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gtst metadata
	fmt.Fprintf(c.printer, "Welcome to the Gtst JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + tst.coinbase);
		console.log("at block: " + tst.blockNumber + " (" + new Date(1000 * tst.getBlock(tst.blockNumber).timestamp) + ")");
