...
func newTester(t *testing.T, confOverride func(*tst.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Tstchain service within
	...
	tstConf := &tst.Config{
		...
		Tsterbase: common.HexToAddress(testAddress),
		Tstash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(tstConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return tst.New(ctx, tstConf) }); err != nil {
		t.Fatalf("failed to register Tstchain protocol: %v", err)
	}
