...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.tststats == "" {
		log.Error("No tststats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Tstash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
	...
	if infos.tststats == "" {
		...
		infos.tststats = w.readString() + ":" + w.conf.tststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.tststats)
		infos.tststats = w.readDefaultString(infos.tststats) + ":" + w.conf.tststats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Tstash != nil {
			// Tstash based miners only need an tsterbase to mine against
			fmt.Println()
			if infos.tsterbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.tsterbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.tsterbase)
				infos.tsterbase = w.readDefaultAddress(common.HexToAddress(infos.tsterbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Tstchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
