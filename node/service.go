...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (tstdb.Database, error) {
	if ctx.config.DataDir == "" {
		return tstdb.NewMemDatabase()
	}
	db, err := tstdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
