...
var (
	...
	configPrefix   = []byte("tstereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db tstdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db tstdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db tstdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db tstdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db tstdb.Putter, count uint64) error {
...
func WriteHeader(db tstdb.Putter, header *types.Header) error {
...
func WriteBody(db tstdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db tstdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db tstdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db tstdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db tstdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db tstdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db tstdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db tstdb.Database) tstdb.Database {
	return tstdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db tstdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db tstdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db tstdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
