...
FROM pupptst/explorer:latest

ADD tststats.json /tststats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../tst-net-intelligence-api && pm2 start /tststats.json)' >  explorer.sh && \
	echo '(cd ../tsterchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerTststats is the configuration file for the tststats javascript client.
var explorerTststats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.tstereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Tststats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	tststats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerTststats)).Execute(tststats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.tststats[:strings.Index(config.tststats, ":")],
		"Secret": config.tststats[strings.Index(config.tststats, ":")+1 : strings.Index(config.tststats, "@")],
		"Host":   config.tststats[strings.Index(config.tststats, "@")+1:],
	})
	files[filepath.Join(workdir, "tststats.json")] = tststats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.tstereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		tststats: infos.envvars["STATS"],
	}
	return stats, nil
}
