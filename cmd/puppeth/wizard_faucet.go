...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Tsters to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.tststats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.tststats = w.readString() + ":" + w.conf.tststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.tststats)
		infos.node.tststats = w.readDefaultString(infos.node.tststats) + ":" + w.conf.tststats
	}
