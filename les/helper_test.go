...
import (
	...
	"github.com/tstchain/go-tstchain/les/flowcontrol"
...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db tstdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := tst.NewBloomIndexer(db, params.BloomBitsBlocks)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db tstdb.Database) *ProtocolManager {
