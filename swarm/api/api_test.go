...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.tst"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.tst"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.tst"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	tstAddr := "swarm.tst"
	tstHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	tstResolve := newTestResolver(tstHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(tstResolve, "")),
			addr:   tstAddr,
			result: tstHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(tstResolve, ""),
				MultiResolverOptionWithResolver(tstResolve, ""),
			),
			addr:   tstAddr,
			result: tstHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(tstResolve, "tst"),
			),
			addr:   tstAddr,
			result: tstHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "tst"),
				MultiResolverOptionWithResolver(doesntResolve, "tst"),
				MultiResolverOptionWithResolver(tstResolve, "tst"),
			),
			addr:   tstAddr,
		...
		},
		{
			...
			addr:   tstAddr,
			result: tstHash,
			err:    errors.New(`DNS name not found: "swarm.tst"`),
		},
		{
