...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for tststats availability")
	if infos, err := checkTststats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["tststats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["tststats"] = infos.Report()
		tststats = infos.config
	}
