...
FROM pupptst/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gtst --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gtst --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --tststats \'{{.Tststats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PupptstNetworkID/{{.NetworkID}}/g' dist/js/tsterwallet-master.js && \
	sed -i 's/PupptstNetwork/{{.Network}}/g'     dist/js/tsterwallet-master.js && \
	sed -i 's/PupptstDenom/{{.Denom}}/g'         dist/js/tsterwallet-master.js && \
	sed -i 's/PupptstHost/{{.Host}}/g'           dist/js/tsterwallet-master.js && \
	sed -i 's/PupptstRPCPort/{{.RPCPort}}/g'     dist/js/tsterwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whtster
// it's running, and if yes, whtster it's responsive.
