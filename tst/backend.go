...
// Package tst implements the Tstchain protocol.
package tst
...
func New(ctx *node.ServiceContext, config *Config) (*Tstchain, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run tst.Tstchain in light sync mode, use les.LightTstchain")
	}
...
func (s *Tstchain) Tsterbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (self *Tstchain) SetTsterbase(tsterbase common.Address) {
	self.lock.Lock()
...
func (s *Tstchain) ChainDb() tstdb.Database            { return s.chainDb }
func (s *Tstchain) IsListening() bool                  { return true } // Always listening
func (s *Tstchain) TstVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
