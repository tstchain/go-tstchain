...
func init() {
	// Run the app if we've been exec'd as "gtst-test" in runGtst.
...
func runGtst(t *testing.T, args ...string) *testgtst {
	tt := &testgtst{}
	...
	for i, arg := range args {
		...
		case arg == "-tsterbase" || arg == "--tsterbase":
			if i < len(args)-1 {
				tt.Tsterbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
		...
		// Remove the temporary datadir if somtsting fails below.
		defer func() {
			if t.Failed() {
				tt.Cleanup()
			}
