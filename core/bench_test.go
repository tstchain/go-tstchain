...
// genTxRing returns a block generator that sends tster in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db tstdb.Database
	if !disk {
		db, _ = tstdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "tst-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = tstdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db tstdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "tst-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := tstdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
