...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (tst/61) or header (tst/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (tst/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both tst/61 and tst/62)
}
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somtsting went very wrong, drop the peer
