...
func (n *Node) OpenDatabase(name string, cache, handles int) (tstdb.Database, error) {
	if n.config.DataDir == "" {
		return tstdb.NewMemDatabase()
	}
	return tstdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
