...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for TstSubscribe requests
}
...
// TstSubscribe registers a subscripion under the "tst" namespace.
func (c *Client) TstSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "tst", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. TstSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through TstSubscribe.
