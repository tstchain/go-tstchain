...
import (
	...
	"github.com/tstchain/go-tstchain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gtst console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = tst.getBlock(tst.getTransaction(tx).blockHash);
  var genesis = tst.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.tst.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  tst.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
