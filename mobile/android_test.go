...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gtst.LightScryptN, Gtst.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tstereum", new NodeConfig());
			node.start();
			...
			TstchainClient ec = node.getTstchainClient();
	...
	// signatures too. Regression test for go-tstereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogtster
	...
	workspace, err := ioutil.TempDir("", "gtst-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/tstereum/gtsttest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gtst and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.tstereum", "github.com/tstchain/go-tstchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gtst.aar"), "gtst.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tstereum", "gtsttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
