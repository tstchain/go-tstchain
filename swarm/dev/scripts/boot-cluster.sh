...
#
# The cluster contains a bootnode, a gtst node and multiple swarm nodes, with
...
# static gtst configuration
GTST_IP="192.168.33.3"
GTST_RPC_PORT="8545"
GTST_RPC_URL="http://${GTST_IP}:${GTST_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gtst and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gtst node with --datadir pointing at <base-dir>/gtst
# and a single, unlocked account with password "gtst"
start_geth_node() {
  local dir="${base_dir}/gtst"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gtst account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GTST_IP}"
    --rpcport   "${GTST_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gtst" "${GTST_IP}" "$(which gtst)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vtst pair
  local vtst0="vtst${name}0"
  local vtst1="vtst${name}1"
  ip link add name "${vtst0}" type vtst peer name "${vtst1}"

  # add one end to the bridge
  ip link set dev "${vtst0}" master "${BRIDGE_NAME}"
  ip link set dev "${vtst0}" up

  # add the other end to the namespace, rename it tst0 and give it the ip
  ip link set dev "${vtst1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vtst1}" name "tst0"
  ip netns exec "${name}" ip link set dev "tst0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "tst0"
}
...
create_account() {
  ...
  gtst --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
