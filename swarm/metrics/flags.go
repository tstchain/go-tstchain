...
import (
	...
	gtstmetrics "github.com/tstchain/go-tstchain/metrics"
	"github.com/tstchain/go-tstchain/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gtstmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gtstmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
