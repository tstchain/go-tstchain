...
import (
	...
	"github.com/tstchain/go-tstchain/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togtster.
//
...
// Connect connects two nodes togtster by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whtster there has been recent attempt to connect the peers
...
type Node struct {
	...
	// Up tracks whtster or not the node is running
	Up bool `json:"up"`
}
...
type Conn struct {
	...
	// Up tracks whtster or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whtster both nodes are currently up
