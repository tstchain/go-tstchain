...
// Package core implements the Tstchain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whtster to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     tstdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Tstchain Validator and
// Processor.
func NewBlockChain(db tstdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= tstdb.IdealBatchSize {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nontsteless.
...
//
// The verify parameter can be used to fine tune whtster nonce verification
